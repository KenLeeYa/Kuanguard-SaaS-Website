...
export function proxy(request: NextRequest) {
  ...
  const internalApi = path === "/api/internal" || path.startsWith("/api/internal/") || path === "/internal" || path.startsWith("/internal/");
  ...
  if (host === "app.kuanguard.com" && path === "/") { url.pathname = "/dashboard"; return NextResponse.redirect(url); }
  const response = NextResponse.next();
  if (isAdmin || internalApi || path.startsWith("/api/") || ["dashboard", "projects", "calendar", "assets", "findings", "reports", "phishing", "training", "wallet", "quotes", "contracts", "orders", "organization", "support", "learn", "login", "onboarding", "entitlements", "questionnaires", "notifications"].includes(root)) {
