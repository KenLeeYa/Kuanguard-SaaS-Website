...
import { LanguageSwitcher } from "./website-language";
import { splitLocale } from "@/lib/locales";
import { platformEntries } from "@/lib/product-links";

const products = [
  { id: "01", category: "MERCHANT SAAS", name: "攤點通｜餐飲點餐系統", description: "從掃碼點餐、廚房出單到門市管理，讓每一天的營運更有條理。", href: "/products/ordering", action: "探索點餐產品", kind: "ordering", tags: ["線上點餐", "廚房出單", "門市管理"] },
...
function Eyebrow({ children }: { children: React.ReactNode }) {
  return <Localized><span className="kg-eyebrow"><span aria-hidden="true" />{children}</span></Localized>;
}
function Action({ href, children, secondary = false }: { href: string; children: React.ReactNode; secondary?: boolean }) {
  return <Localized><Link className={`kg-button${secondary ? " kg-button-secondary" : ""}`} href={href}>{children}<ArrowUpRight size={17} aria-hidden="true" /></Link></Localized>;
}
  ...
  return <Localized><header className="kg-header" onKeyDown={event => { if (event.key === "Escape" && open) { setOpen(false); menuButton.current?.focus(); } }}>
    <div className="kg-container kg-header-inner">
      ...
      <nav id="corporate-navigation" className={open ? "is-open" : ""} aria-label="主要導覽" onClick={event => { if (event.target instanceof Element && event.target.closest("a")) setOpen(false); }}>
        ...
        <LanguageSwitcher /><span className="kg-nav-actions"><Link href="/login">平台登入<ArrowUpRight size={13} aria-hidden="true" /></Link><Action href="/contact">聯絡我們</Action></span>
      </nav>
    </div>
  </header></Localized>;
}
...
function TechnologyVisual() {
  return <Localized><div className="kg-technology" aria-label="KUANGUARD 旗下產品與服務架構">
...
function ProductVisual({ kind }: { kind: string }) {
  return <Localized><div className={`kg-product-visual kg-visual-${kind}`} aria-hidden="true">
...
function ProductCards() {
  return <Localized><div className="kg-product-grid">{products.map(product => <article className="kg-product-card" key={product.id}>
    <ProductVisual kind={product.kind} /><div className="kg-product-copy"><div className="kg-product-category"><span>{product.category}</span><span>{product.id}</span></div><h3>{product.name}</h3><p>{product.description}</p><div className="kg-product-tags">{product.tags.map(tag => <span key={tag}>{tag}</span>)}</div><div className="kg-card-actions"><Link className="kg-card-link" href={product.href}>{product.action}<ArrowUpRight size={17} aria-hidden="true" /></Link>{platformEntries.find(entry => entry.id === product.kind)?.login ? <a className="kg-card-link" href={platformEntries.find(entry => entry.id === product.kind)!.login!}>商家登入<ArrowUpRight size={17} aria-hidden="true" /></a> : <span className="kg-platform-status">平台尚未開放</span>}</div></div>
  </article>)}</div></Localized>;
}
function AudienceRows() {
  return <Localized><div className="kg-audiences">{audiences.map(({ id, title, text, href, label, icon: Icon }) => <Link className="kg-audience-row" href={href} key={id}><span className="kg-row-number">{id}</span><div className="kg-audience-title"><Icon size={23} strokeWidth={1.5} aria-hidden="true" /><h3>{title}</h3></div><p>{text}</p><span className="kg-audience-link">{label}<ArrowUpRight size={17} aria-hidden="true" /></span></Link>)}</div></Localized>;
}
function Closing() {
  return <Localized><section className="kg-closing"><div className="kg-container"><div><Eyebrow>LET’S BUILD WHAT’S NEXT</Eyebrow><h2>你的下一個數位計畫，<br />從一次對話開始。</h2><p>告訴我們你的使用情境，一起找到合適的產品與服務。</p></div><Action href="/contact">與 KUANGUARD 聯繫</Action></div></section></Localized>;
}
export function CorporateHome() {
  return <Localized><div className="kg-site">
...
export function CorporateProducts() {
  return <Localized><div className="kg-site"><section className="kg-page-intro kg-container"><Eyebrow>KUANGUARD / PRODUCTS & PLATFORMS</Eyebrow><h1>讓數位工具，<br /><em>成為工作的助力。</em></h1><p>KUANGUARD 旗下產品與服務，從商家營運到企業安全與夥伴協作。<br />選擇適合的起點，了解每一項產品的功能與導入方式。</p></section><section className="kg-container kg-catalog-section"><ProductCards /><p className="kg-catalog-note">點餐系統為 KUANGUARD 旗下商家 SaaS 產品；收費與功能狀態請見產品專頁。<br />企業資安由專業服務與核定交付流程提供，Partner 平台規劃合作夥伴管理與協作，尚未開放登入。</p><aside className="kg-planned-product" id="beauty"><span className="kg-platform-status">規劃中</span><h2>美業工作室系統</h2><p>將以獨立產品規劃，目前尚未上線；攤點通專注餐飲點餐。</p><Link href="/contact">洽詢產品規劃<ArrowUpRight size={16} aria-hidden="true" /></Link></aside></section><Closing /></div></Localized>;
}
export function CorporateSolutions() {
  return <Localized><div className="kg-site"><section className="kg-page-intro kg-container"><Eyebrow>KUANGUARD / SOLUTIONS</Eyebrow><h1>好的數位方案，<br /><em>從理解工作開始。</em></h1><p>先釐清你的使用情境，再選擇需要的產品、服務與協作方式。</p></section><section className="kg-container kg-catalog-section"><AudienceRows /><div className="kg-adoption"><Eyebrow>HOW WE GET STARTED</Eyebrow><h2>一步一步，把需求帶進實際工作。</h2><ol>{["了解使用情境", "確認產品與服務範圍", "安排導入與驗收", "持續服務與協作"].map((step, i) => <li key={step}><span>0{i + 1}</span>{step}</li>)}</ol></div></section><Closing /></div></Localized>;
}
