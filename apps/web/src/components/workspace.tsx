...
export function WorkspacePage({ path }: { path: string }) { const { session, loading, error, refresh } = useSession(); const [navOpen, setNavOpen] = useState(false); const logout = useMutation(); const router = useRouter(); const internal = path.startsWith("admin"); const learner = path.startsWith("learn"); const fullPath = path === "assets" ? "/findings" : `/${path}`; const nav = internal ? internalNav : learner ? learnerNav : customerNav;
  ...
  const visibleNav = nav.filter(n => allowsNav(session.roles, n) && (!session.delegated || !["/admin/portfolio", "/admin/integrations", "/admin/crm", "/admin/administration/lifecycle"].some(prefix => n.href.startsWith(prefix))));
  return <div className="workspace">{navOpen && <button className="sidebar-overlay" aria-label="關閉側欄" onClick={() => setNavOpen(false)} />}<aside className={`sidebar ${navOpen ? "is-open" : ""}`}><div className="portal-workspace-brand">{session.partner_branding ? <strong>{session.partner_branding.branding?.company_name || session.partner_branding.name}</strong> : <Brand light />}</div><p className="sidebar-caption">{internal ? "INTERNAL WORKSPACE" : learner ? "MY LEARNING" : "CUSTOMER WORKSPACE"}</p><nav className="side-nav" aria-label="工作空間導覽">{visibleNav.map(n => { const Icon = n.icon; const active = bestMatch?.href === n.href; return <div key={n.href}>{n.group && <div className="nav-group">{n.group}</div>}<Link className={active ? "active" : ""} href={n.href} aria-current={active ? "page" : undefined} onClick={() => setNavOpen(false)}><Icon size={17} />{n.label}</Link></div>; })}</nav>{session.platform_admin && !session.delegated && <Link className="portal-sidebar-back" href="/admin/platform">平台組織與商家管理</Link>}<div className="sidebar-bottom"><ShieldCheck size={21} /><div><strong>您的資料，依權限守護。</strong>租戶隔離 · 操作留存</div></div></aside><div className="workspace-main"><header className="workspace-topbar"><div className="workspace-topbar-left"><button className="icon-button mobile-sidebar-toggle" aria-label="開啟工作選單" aria-expanded={navOpen} onClick={() => setNavOpen(!navOpen)}><Menu size={18} /></button><strong>{internal ? "內部作業" : learner ? "個人學習" : "企業工作空間"}</strong><span>/</span><span>{bestMatch?.label || "工作台"}</span></div><div className="workspace-topbar-right"><Link className="icon-button" aria-label="我的通知" href={internal ? "/admin/notifications" : learner ? "/learn/notifications" : "/notifications"}><Bell size={17} /></Link><span className="tenant-label"><Building2 size={14} />{session.tenant.name}</span><div className="user-meta"><div className="user-avatar">{session.user.name.slice(0, 1)}</div><span>{session.user.name}</span></div><button className="logout-button" disabled={logout.pending} onClick={async () => { if (!confirmFormNavigation()) return; if (await logout.mutate("/auth/logout")) { await refresh(); router.push("/login"); } }}><LogOut size={13} />登出</button></div></header>{session.delegated && <DelegationBar />}{session.development && <div className="development-banner"><AlertCircle size={13} /><span>本機開發環境 · 合成資料 · 正式寄送、付款與外部服務尚未啟用</span></div>}<main id="main-content" className="workspace-content">{permitted ? <WorkspaceContent path={path} /> : <Card><ErrorState error={new ApiError(403, "FORBIDDEN", "目前角色未獲授權使用此功能。請返回您的工作空間。") } /><div className="success-panel"><ButtonLink href={homeFor(session.roles)}>返回我的工作空間</ButtonLink></div></Card>}<footer className="workspace-footer"><span>{session.partner_branding?.branding?.footer || "KUANGUARD · 企業資安服務平台"}</span><span>時間顯示：Asia/Taipei · 權限依企業與角色</span></footer></main></div></div>;
}
...
function DelegationBar() { const action = useMutation(); const { refresh, session } = useSession(); const router = useRouter(); return <div className="portal-delegation-bar"><span>目前服務客戶：{session?.tenant.name} · 個人授權與專案限制仍生效</span><button className="button button-secondary" disabled={action.pending} onClick={async () => { if (!confirmFormNavigation()) return; if (await action.mutate("/partner/return")) { await refresh(); router.push("/partner"); } }}>返回合作夥伴工作台</button><MutationStatus action={{ ...action, result: null }} /></div>; }
